...
Example: `/add-quizzes kubernetes`

## Flow

### Step 1: DNA Gate
1. Check if `knowledge/LearningDNA.md` exists — if missing, run DNA Creation Flow (see /new-topic skill for the full questionnaire)
...
- If no source files exist (or only overview.md), suggest running `/research {topic} <subtopic>` first
